...
const EditOrderModal: React.FC<EditOrderModalProps> = ({ isOpen, onClose, onEdit, isLoading, order }) => {
  const [orderNumber, setOrderNumber] = useState('');
  const [customerName, setCustomerName] = useState('');
  const [status, setStatus] = useState<OrderStatus>('pending');
  const [createdAt, setCreatedAt] = useState('');
  const [shippedTime, setShippedTime] = useState<string>('');
  ...
  useEffect(() => {
    if (isOpen && order) {
      ...
      setCreatedAt(format(order.createdAt, 'yyyy-MM-dd'));
      setItems(order.items.map(item => ({ ...item }))); // preserve ids
      setShippingAddress(order.shippingAddress);
      setBillingAddress(order.billingAddress);
      setPaymentMethod(order.paymentMethod);
      setNotes(order.notes || '');
      if (order.status === 'shipped' && order.shippedTime) {
        setShippedTime(format(order.shippedTime, "yyyy-MM-dd'T'HH:mm"));
      } else {
        setShippedTime('');
      }
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedOrderData: Omit<Order, 'id' | 'createdAt' | 'updatedAt' | 'shippedTime'> & { shippedTime?: Date | null } = {
      orderNumber,
      customerName,
      status,
      items: items.map((item, idx) => {
        let id: string;
        if (typeof (item as { id?: string }).id === 'string' && (item as { id?: string }).id) {
    ...
    if (notes) updatedOrderData.notes = notes;

    if (status === 'shipped' && shippedTime) {
      updatedOrderData.shippedTime = parseISO(shippedTime);
    } else if (order?.shippedTime && status !== 'shipped') {
      updatedOrderData.shippedTime = null;
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Order" size="xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Select label="Status" value={status} onChange={e => setStatus(e.target.value as OrderStatus)} options={statusOptions} required fullWidth />
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" isLoading={isLoading} variant="primary">Save Changes</Button>
        </div>
      </form>
    </Modal>
  );
